<?php
/**
 * Partial: Configuración SRI (panel dentro del hub de facturación).
 *
 * Datos del emisor, certificado digital y puntos de emisión. Se incluye
 * desde admin/views/billing.php.
 *
...
<div class="af-sri-hero">
	...
	<div class="af-sri-hero__links">
		<a class="button af-btn af-btn--ghost" href="#af-sri-datos-emisor"><?php esc_html_e( '1. Datos del emisor', 'arriendo-facil' ); ?></a>
		<a class="button af-btn af-btn--ghost" href="#af-sri-certificado"><?php esc_html_e( '2. Certificado', 'arriendo-facil' ); ?></a>
		<?php if ( ! $af_is_owner ) : ?>
			<a class="button af-btn af-btn--ghost" href="#af-sri-puntos"><?php esc_html_e( '3. Puntos de emisión', 'arriendo-facil' ); ?></a>
		<?php endif; ?>
	</div>
...
<div class="af-sri-section" id="af-sri-datos-emisor">
	...
	<form method="post" action="" class="af-sri-form">
		...
		<div class="af-form-grid">
			<div class="af-form-field af-form-field--full">
				<label class="af-form-field__label" for="af_ruc">
					<?php esc_html_e( 'RUC', 'arriendo-facil' ); ?> <span class="af-required">*</span>
					<span class="af-form-field__hint">(13 dígitos; o cédula de 10 dígitos para persona natural)</span>
				</label>
				<div class="af-input-row">
					...
					<button type="button" id="af-ruc-lookup-btn" class="button af-btn af-btn--ghost">
						<?php echo af_lucide( 'search', 15 ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- static SVG helper. ?>
						<?php esc_html_e( 'Consultar en SRI', 'arriendo-facil' ); ?>
					</button>
					<span id="af-ruc-lookup-status" style="font-style:italic; color: var(--af-gray-500, #7A7870);"></span>
				</div>
			</div>

			<div class="af-form-field">
				<label class="af-form-field__label" for="af_razon_social">
					<?php esc_html_e( 'Razón Social', 'arriendo-facil' ); ?> <span class="af-required">*</span>
				</label>
				...
				<span class="af-form-field__hint"><?php esc_html_e( 'Tal como aparece en el RUC del SRI. Auto-completado al consultar el RUC.', 'arriendo-facil' ); ?></span>
			</div>

			<div class="af-form-field">
				<label class="af-form-field__label" for="af_nombre_comercial">
					<?php esc_html_e( 'Nombre Comercial', 'arriendo-facil' ); ?>
					<span class="af-form-field__hint"><?php esc_html_e( 'Opcional. Solo si es diferente a la razón social.', 'arriendo-facil' ); ?></span>
				</label>
			...
			<div class="af-form-field">
				<label class="af-form-field__label" for="af_dir_establecimiento">
					<?php esc_html_e( 'Dirección del Establecimiento', 'arriendo-facil' ); ?> <span class="af-required">*</span>
				</label>
			...
			<div class="af-form-field">
				<label class="af-form-field__label" for="af_dir_matriz">
					<?php esc_html_e( 'Dirección Matriz', 'arriendo-facil' ); ?>
				</label>
				...
				<input type="text" id="af_dir_matriz" name="af_dir_matriz"
					...
					class="large-text" maxlength="300" <?php echo esc_attr( $af_readonly_attr ); ?> />
				<span class="af-form-field__hint" id="af-dir-matriz-desc">
					<?php if ( $af_dir_match ) : ?>
						<span style="color:var(--af-success-700, #2E7D32); font-weight:500;">✓ <?php esc_html_e( 'Igual a la dirección del establecimiento.', 'arriendo-facil' ); ?></span>
					<?php else : ?>
			...
			<div class="af-form-field af-form-field--full">
				<span class="af-form-field__label">
					<?php esc_html_e( 'Obligado a llevar Contabilidad', 'arriendo-facil' ); ?> <span class="af-required">*</span>
				</span>
				...
				<span class="af-form-field__hint"><?php esc_html_e( 'Auto-completado al consultar el RUC.', 'arriendo-facil' ); ?></span>
			</div>

			<div class="af-form-field af-form-field--full">
				<label class="af-form-field__label" for="af_email_notificacion">
					<?php esc_html_e( 'Email para notificaciones', 'arriendo-facil' ); ?> <span class="af-required">*</span>
				</label>
				...
				<span class="af-form-field__hint" style="color:var(--af-danger-700, #C62828); font-weight:500;">
				...
				<span id="af-email-error" class="af-form-field__error" style="display:none;"></span>
			</div>
		</div>

		<h2 class="af-sri-form__title"><?php esc_html_e( 'Ambiente SRI', 'arriendo-facil' ); ?></h2>
		<div class="af-form-field af-form-field--full" style="margin-top: var(--af-space-2);">
			<span class="af-form-field__label"><?php esc_html_e( 'Ambiente de emisión', 'arriendo-facil' ); ?></span>
			<?php if ( $af_is_owner ) : ?>
				<div class="af-radio-row">
					<label>
						<input type="radio" name="af_ambiente_display" value="2" checked disabled />
						<strong><?php esc_html_e( 'Producción', 'arriendo-facil' ); ?></strong>
						<span class="af-form-field__hint"> — <?php esc_html_e( 'Comprobantes con validez legal ante el SRI.', 'arriendo-facil' ); ?></span>
					</label>
				</div>
				<span class="af-form-field__hint"><?php esc_html_e( 'Los owners emiten siempre en ambiente de producción.', 'arriendo-facil' ); ?></span>
			<?php else : ?>
				<div class="af-radio-row">
					<label>
						...
						<span class="af-form-field__hint"> — <?php esc_html_e( 'Servidor de certificación SRI. Sin validez tributaria.', 'arriendo-facil' ); ?></span>
					</label>
					<label>
						<input type="radio" name="af_ambiente" value="2"
							<?php checked( $cfg['ambiente'], '2' ); ?> />
						<strong><?php esc_html_e( 'Producción', 'arriendo-facil' ); ?></strong>
						<span class="af-form-field__hint"> — <?php esc_html_e( 'Servidor productivo SRI. Comprobantes con validez legal.', 'arriendo-facil' ); ?></span>
					</label>
				</div>
			<?php endif; ?>
		</div>
		...
		<?php if ( ! $af_is_owner ) : ?>
		<h2 class="af-sri-form__title" style="margin-top: var(--af-space-5);">
			<?php esc_html_e( 'Configuración Avanzada', 'arriendo-facil' ); ?>
			<span class="af-form-field__hint">(<?php esc_html_e( 'Opcional', 'arriendo-facil' ); ?>)</span>
		</h2>
		<p class="af-sri-form__desc">
		...
		<div class="af-form-grid">
			<div class="af-form-field">
				<label class="af-form-field__label" for="af_sri_soap_timeout">
					<?php esc_html_e( 'Timeout SOAP (segundos)', 'arriendo-facil' ); ?>
				</label>
				...
				<span class="af-form-field__hint"><?php esc_html_e( '⚙️ Default: 30 seg. Aumenta a 60 si tu conexión es lenta.', 'arriendo-facil' ); ?></span>
			</div>
			<div class="af-form-field">
				<label class="af-form-field__label" for="af_sri_soap_max_retries">
					<?php esc_html_e( 'Máximo de Reintentos', 'arriendo-facil' ); ?>
				</label>
				...
				<span class="af-form-field__hint"><?php esc_html_e( '⚙️ Default: 3. Reintentos inmediatos ante errores temporales.', 'arriendo-facil' ); ?></span>
			</div>
		</div>
		<?php endif; ?>

		<div class="af-sri-form__actions">
			<button type="submit" name="af_save_sri_config" class="button af-btn af-btn--primary">
				<?php echo af_lucide( 'save', 16 ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- static SVG helper. ?>
...
<div class="af-sri-section" id="af-sri-certificado">
	...
	<div class="af-cert-status">
		<span class="af-cert-status__item <?php echo $cert_path ? 'is-ok' : 'is-warn'; ?>">
			<span class="af-status-chip__dot" aria-hidden="true"></span>
			<?php
			if ( $cert_path ) {
				echo esc_html( __( 'Certificado cargado', 'arriendo-facil' ) . ' · ' . basename( $cert_path ) );
			} else {
				esc_html_e( 'Ningún certificado cargado', 'arriendo-facil' );
			}
			?>
		</span>
		<span class="af-cert-status__item <?php echo $cert_has_pwd ? 'is-ok' : 'is-warn'; ?>">
			<span class="af-status-chip__dot" aria-hidden="true"></span>
			<?php echo $cert_has_pwd ? esc_html__( 'Contraseña guardada', 'arriendo-facil' ) : esc_html__( 'Sin contraseña', 'arriendo-facil' ); ?>
		</span>
	</div>

	<form method="post" action="" enctype="multipart/form-data" class="af-sri-form">
		<?php wp_nonce_field( 'af_sri_settings_nonce' ); ?>
		<div class="af-form-grid">
			<div class="af-form-field">
				<label class="af-form-field__label" for="af_cert_file">
					<?php esc_html_e( 'Archivo .p12 / .pfx', 'arriendo-facil' ); ?>
				</label>
				<input type="file" id="af_cert_file" name="af_cert_file" accept=".p12,.pfx" />
				<span class="af-form-field__hint">
			...
			<div class="af-form-field">
				<label class="af-form-field__label" for="af_cert_password">
					<?php esc_html_e( 'Contraseña del certificado', 'arriendo-facil' ); ?>
				</label>
		...
		<div class="af-sri-form__actions">
			<button type="submit" name="af_upload_certificate" class="button af-btn af-btn--primary">
				<?php echo af_lucide( 'upload', 16 ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- static SVG helper. ?>
	...
	<?php if ( $cert_has_pems ) : ?>
		<form method="post" action="" class="af-sri-form" style="margin-top: var(--af-space-4);">
			<?php wp_nonce_field( 'af_sri_settings_nonce' ); ?>
			<div class="af-sri-form__actions">
				<button type="submit" name="af_test_certificate" class="button af-btn af-btn--ghost">
					<?php echo af_lucide( 'shield-check', 15 ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- static SVG helper. ?>
					<?php esc_html_e( 'Verificar certificado', 'arriendo-facil' ); ?>
				</button>
				<button type="submit" name="af_rebuild_chain" class="button af-btn af-btn--ghost">
					<?php echo af_lucide( 'link-2', 15 ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- static SVG helper. ?>
					<?php esc_html_e( 'Reconstruir cadena CA', 'arriendo-facil' ); ?>
				</button>
			</div>
			<p class="af-sri-form__desc" style="margin-top: var(--af-space-2);">
				<?php esc_html_e( 'Verifica que el certificado sea válido y esté activo. Usa "Reconstruir cadena CA" si los certificados intermedios están en 0 o la firma es rechazada por el SRI.', 'arriendo-facil' ); ?>
			</p>
		</form>
	<?php endif; ?>
</div>
			...
			<?php foreach ( $emission_points as $pt ) : ?>
				<tr>
					...
					<td data-label="<?php esc_attr_e( 'Activo', 'arriendo-facil' ); ?>">
						<?php if ( $pt->activo ) : ?>
							<span style="color:var(--af-success-700, #2E7D32);">&#10003;</span>
						<?php else : ?>
							<span style="color:var(--af-danger-700, #C62828);">&#10007;</span>
						<?php endif; ?>
					</td>
				</tr>
			<?php endforeach; ?>
...
<?php endif; ?>
	...
	<h3 class="af-sri-form__title" style="margin-top: var(--af-space-5);"><?php esc_html_e( 'Agregar / Actualizar Punto de Emisión', 'arriendo-facil' ); ?></h3>
	<form method="post" action="" class="af-sri-form">
		<?php wp_nonce_field( 'af_sri_settings_nonce' ); ?>
		<div class="af-form-grid" style="max-width:640px;">
			<div class="af-form-field">
				<label class="af-form-field__label" for="af_cod_estab">
					<?php esc_html_e( 'Código Establecimiento', 'arriendo-facil' ); ?>
				</label>
				<div class="af-input-row">
					<input type="text" id="af_cod_estab" name="af_cod_estab"
						value="001" class="small-text" maxlength="3" pattern="\d{3}" required />
					<span class="af-form-field__hint"><?php esc_html_e( '3 dígitos (ej: 001)', 'arriendo-facil' ); ?></span>
				</div>
			</div>
			<div class="af-form-field">
				<label class="af-form-field__label" for="af_cod_punto">
					<?php esc_html_e( 'Código Punto de Emisión', 'arriendo-facil' ); ?>
				</label>
				<div class="af-input-row">
			...
			<div class="af-form-field af-form-field--full">
				<label class="af-form-field__label" for="af_punto_desc">
					<?php esc_html_e( 'Descripción', 'arriendo-facil' ); ?>
				</label>
			...
			<div class="af-form-field af-form-field--full">
				<label class="af-form-field__label" style="font-weight:400; cursor:pointer; display:inline-flex; gap:8px;">
		...
		<div class="af-sri-form__actions">
			<button type="submit" name="af_save_emission_point" class="button af-btn af-btn--primary">
				<?php echo af_lucide( 'plus', 16 ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- static SVG helper. ?>
...
(function () {
	...
	btn.addEventListener( 'click', function () {
		...
		if ( ruc.length !== 13 ) {
			status.innerHTML = '<span style="color:var(--af-danger-700, #C62828);">Ingresa el RUC (13 dígitos) o tu cédula (10 dígitos).</span>';
			return;
		}
		btn.disabled = true;
		status.innerHTML = '<span style="color:var(--af-gray-600, #55556B);">Consultando SRI…</span>';
		...
		.then( function ( resp ) {
			if ( resp.success && resp.data ) {
				...
				status.innerHTML = '<span style="color:var(--af-success-700, #2E7D32);">✓ Datos cargados desde el SRI. Revisa y guarda.</span>';
			} else {
				var msg = ( resp.data && resp.data.message ) ? resp.data.message : 'No se encontró información.';
				status.innerHTML = '<span style="color:var(--af-danger-700, #C62828);">' + msg + '</span>';
			}
		} )
		.catch( function ( err ) {
			status.innerHTML = '<span style="color:var(--af-danger-700, #C62828);" title="' + ( err.message || '' ).replace( /"/g, '&quot;' ) + '">Error al consultar el SRI. Detalle: ' + ( err.message ? err.message.substring( 0, 120 ) : 'Error de red' ) + '</span>';
		} )
