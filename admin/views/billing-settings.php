				...
				if ( (int) $chain_count > 0 ) {
					...
					$af_sri_notice = array(
						...
						'msg'  => __(
							...
							'Tu servidor podría estar bloqueando salidas HTTPS a los servidores BCE/SecurityData; ' .
...
// Test certificate (safe output without sensitive details).
if ( isset( $_POST['af_test_certificate'] ) ) {
	...
	if ( is_wp_error( $test_result ) ) {
		$af_sri_notice = array(
			'type' => 'error',
			'msg'  => __( 'No se pudo validar el certificado. Verifica que el archivo y la contraseña sean correctos.', 'arriendo-facil' ),
		);
	} else {
		$af_sri_notice = array(
			'type' => 'success',
			'msg'  => __( 'Certificado verificado correctamente.', 'arriendo-facil' ),
		);
	}
}
