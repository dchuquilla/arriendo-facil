...
class Arriendo_Facil_Billing_Manager {
	...
	/**
	 * Builds a preview payload for a lease invoice before actual issuance.
	 *
	 * @param int   $lease_id   Lease ID.
	 * @param array $overrides  Optional payload overrides.
	 * @param array $options    Optional keys: billing_period (YYYY-MM).
	 * @return array|WP_Error
	 */
	public function preview_lease_invoice( int $lease_id, array $overrides = array(), array $options = array() ) {
		$period = isset( $options['billing_period'] )
			? (string) $options['billing_period']
			: self::billing_period();
		...
		$item = isset( $totals['items'][0] ) && is_array( $totals['items'][0] )
			? $totals['items'][0]
			: array();
		...
		if ( $existing_period ) {
			...
			if ( in_array( $estado_existente, $estados_validos, true ) ) {
				...
				$warning   = sprintf(
					/* translators: 1: period YYYY-MM, 2: estado */
					__( 'Ya existe un comprobante para el periodo %1$s (estado: %2$s).', 'arriendo-facil' ),
					$period,
					$estado_existente
				);
			}
		...
		return array(
			...
			'buyer'          => array(
				...
				'tipo_id'        => (string) ( $payload['tipo_id_comprador'] ?? '' ),
			),
			'item'           => array(
				'codigo_principal' => (string) ( $item['codigo_principal'] ?? '' ),
				...
				'precio_total_sin_impuesto' => isset( $item['precio_total_sin_impuesto'] ) ? (float) $item['precio_total_sin_impuesto'] : 0.0,
			),
			'totals'         => array(
				'total_sin_impuestos' => (float) ( $totals['total_sin_impuestos'] ?? 0.0 ),
				'total_descuento'     => (float) ( $totals['total_descuento'] ?? 0.0 ),
				'iva_valor'           => (float) ( $totals['iva_valor'] ?? 0.0 ),
				'importe_total'       => (float) ( $totals['importe_total'] ?? 0.0 ),
				'iva_codigo_porcentaje' => (string) ( $totals['iva_codigo_porcentaje'] ?? $iva_codigo_porcentaje ),
			),
			'payment'        => array(
