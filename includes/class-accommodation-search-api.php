...
class Arriendo_Facil_Accommodation_Search_API {
	...
	private function build_accommodation_from_row( $row ) {
		...
		return array(
			...
			'latitude'              => ( $row->latitude !== null && $row->latitude !== '' && floatval( $row->latitude ) !== 0.0 ) ? floatval( $row->latitude ) : null,
			'longitude'             => ( $row->longitude !== null && $row->longitude !== '' && floatval( $row->longitude ) !== 0.0 ) ? floatval( $row->longitude ) : null,
			'status'                => $status,
